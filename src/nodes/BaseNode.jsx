...
export default function BaseNode({ id, data, type }) {
  const label = data?.label || data?.signalName || type || `Node ${id}`;
  ...
  return (
    ...
    >
      ...
      {/* top input handle */}
      <Handle type="target" position={Position.Top} id="in" style={{ background: "#9ca3af", border: "none", width: 10, height: 10 }} />
      {/* bottom output handle */}
      <Handle type="source" position={Position.Bottom} id="out" style={{ background: "#0969da", border: "none", width: 12, height: 12 }} />
    </div>
  );
}
